...
# Resolve project directory for cross-platform file paths
...
class NeonHealthApp:
    def __init__(self):
        ...
        # Load images
        try:
            self.pancreas_image = Image.open(os.path.join(PROJECT_DIR, "x", "3.png"))
    ...
    def load_and_verify_models(self):
        ...
        try:
            # Load models using project-relative path
            ...
            # Load kidney model (handle possible uppercase filename on disk)
            kidney_model_path = os.path.join(model_dir, 'kidney.pkl')
            kidney_model_path_upper = os.path.join(model_dir, 'KIDNEY.pkl')
            if os.path.exists(kidney_model_path):
                self.kidney_model = joblib.load(kidney_model_path)
                print("Kidney model loaded successfully")
            elif os.path.exists(kidney_model_path_upper):
                self.kidney_model = joblib.load(kidney_model_path_upper)
                print("Kidney model loaded successfully (uppercase filename)")
            else:
    ...
    def show_login(self):
        ...
        login_frame = ctk.CTkFrame(
            self.root,
            fg_color="#e0e6e9",  # تم تغييره من #f5f9f0
            corner_radius=30,
            border_width=2,
            border_color="#00bf63"
        )
    ...
    def show_main_menu(self):
        ...
        container = ctk.CTkFrame(
            self.root,
            fg_color="#e0e6e9",  # تم تغييره من #f1f8e9
            corner_radius=0
        )
        ...
        sidebar = ctk.CTkFrame(
            container,
            fg_color="#00bf63",  # Changed from black to green
            corner_radius=0,
            width=200
        )
        ...
        logout_button = ctk.CTkButton(
            ...
            hover_color="#00bf63",  # Changed hover color to darker green
            text_color="white",
            corner_radius=20,
            height=40,
            width=160,
        ...
        for text, command, tooltip in menu_items:
            button_frame = ctk.CTkFrame(
                sidebar,
                fg_color="#00bf63",  # Changed from #00CD66 to match new green theme
                corner_radius=15,
                height=45
            )
        ...
        nav_buttons = [
            ...
            ("Liver", "LIVER SCAN"),
            ("Blood Disorders", "ANEMIA ANALYSIS"),
        ...
        for i, (button_text, test_type) in enumerate(nav_buttons):
            button = ctk.CTkButton(
                ...
                text_color="white",  # Changed from black to white for better contrast
                border_width=2,
                border_color="#00bf63",  # Changed from black to dark green
        ...
        try:
            ...
            image_path = os.path.join(PROJECT_DIR, "x.png")
            print(f"Loading image from: {image_path}")
            
            try:
                pil_image = Image.open(image_path)
            except:
                image_path = os.path.join(PROJECT_DIR, "x.jpg")
                try:
                    pil_image = Image.open(image_path)
                except:
                    image_path = os.path.join(PROJECT_DIR, "x.jpeg")
                    pil_image = Image.open(image_path)
            ...
            button_configs = [
                {
                    ...
                    "size": (60, 60),
                    "hover_size": (90, 90),
                    "image_path": os.path.join(PROJECT_DIR, "x", "1.png")
                },
                {
                    ...
                    "size": (60, 60),
                    "hover_size": (90, 90),
                    "image_path": os.path.join(PROJECT_DIR, "x", "2.png")
                },
                {
                    "name": "LIVER SCAN",
                    "position": (390, 450),
                    "size": (60, 60),
                    "hover_size": (90, 90),
                    "image_path": os.path.join(PROJECT_DIR, "x", "4.png")
                },
                {
                    ...
                    "size": (60, 60),
                    "hover_size": (90, 90),
                    "image_path": os.path.join(PROJECT_DIR, "x", "5.png")
                },
                {
                    ...
                    "size": (60, 60),
                    "hover_size": (90, 90),
                    "image_path": os.path.join(PROJECT_DIR, "x", "6.png")
                }
            ]
    ...
    #    )
      #  load_data_button.pack(side="left", padx=5)
       ...
       # form_frame = ctk.CTkFrame(
         ...
         #   fg_color="transparent"
      #  )
       # form_frame.pack(expand=True, fill="both", padx=30, pady=(0, 20))
       ...
       # if test_type in ["QUANTUM KIDNEY ANALYSIS", "CARDIAC HOLOGRAM", "LIVER SCAN", "ANEMIA ANALYSIS", "NEURAL DIABETES SCAN"]:
        ...
        scrollable_frame.pack(pady=20, padx=40, fill="both", expand=True)
    ...
    def show_results(self, test_type):
        ...
        results_window.configure(fg_color="#e0e6e9")  # تم تغييره من #f1f8e9
        ...
        is_diagnosed = self.is_positive_diagnosis(probability)
        diagnosis_status = "Diagnosed (Positive)" if is_diagnosed else "Healthy (Negative)"
        ...
        if is_diagnosed:
            image_paths = {
                "ANEMIA ANALYSIS": os.path.join(base_path, "anemia.png"),
                "CARDIAC HOLOGRAM": os.path.join(base_path, "heart.png"),
                "QUANTUM KIDNEY ANALYSIS": os.path.join(base_path, "kidneys.png"),
                "LIVER SCAN": os.path.join(base_path, "liver.png")
            }
        ...
        diagnosis_frame = ctk.CTkFrame(
            main_frame,
            fg_color="#e0e6e9",  # تم تغييره من #e8f5e9
            corner_radius=15
        )
    ...
    def show_new_diagnostic_test(self, initial_data):
        ...
        def analyze():
            try:
                ...
                # Load the diabetes model (project-relative)
                model_path = os.path.join(PROJECT_DIR, "skry2.pkl")
                if os.path.exists(model_path):
    ...
    def is_positive_diagnosis(self, probability):
        """Helper function to determine if diagnosis is positive based on probability threshold"""
