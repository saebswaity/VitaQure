...
class NeonHealthApp:
    ...
    def show_test_details(self, test_type):
        ...
        def load_test_data():
            if test_type == "NEURAL DIABETES SCAN":
                ...
                risk_factors = []
                try:
                    if float(data['Glucose']) > 0.6: risk_factors.append("Elevated glucose levels")
                    if float(data['HbA1c']) > 0.6: risk_factors.append("Elevated HbA1c")
                    if float(data['Insulin']) > 0.6: risk_factors.append("Insulin resistance")
                    if float(data['BMI']) > 0.7: risk_factors.append("High BMI")
                    if float(data['Triglycerides']) > 0.6: risk_factors.append("Elevated triglycerides")
                    if float(data['LDL Cholesterol']) > 0.6: risk_factors.append("High LDL cholesterol")
                    if float(data['HDL Cholesterol']) < 0.4: risk_factors.append("Low HDL cholesterol")
                except Exception as e:
                    print(f"Error calculating risk factors: {str(e)}")
    ...
    def show_results(self, test_type):
        ...
        if test_type == "LIVER SCAN" and hasattr(self, 'prediction_results'):
        ...
        elif test_type == "NEURAL DIABETES SCAN" and hasattr(self, 'prediction_results'):
            # Create result display frame
            result_frame = ctk.CTkFrame(
                container,
                fg_color="#f5f9f0",
            ...
            # Group parameters by category
            parameter_groups = {
                'Biochemistry': ['Glucose', 'Cholesterol', 'Hemoglobin', 'Platelets', 'White Blood Cells', 'Red Blood Cells', 'Hematocrit', 'Mean Corpuscular Volume', 'Mean Corpuscular Hemoglobin', 'Mean Corpuscular Hemoglobin Concentration', 'Insulin', 'BMI', 'Systolic Blood Pressure', 'Diastolic Blood Pressure', 'Triglycerides', 'HbA1c', 'LDL Cholesterol', 'HDL Cholesterol', 'ALT', 'AST', 'Heart Rate', 'Creatinine', 'Troponin', 'C-reactive Protein'],
                'Medical History': ['Previous Acute Kidney Injury', 'Diuretics', 'Edema', 'Nausea/Vomiting', 'Muscle Cramps', 'Itching']
            }
            ...
            if is_at_risk:
                rec_text = (
                    "• Schedule an appointment with a diabetes specialist\n"
    ...
    def load_test_data(self):
        if self.current_test_type == "NEURAL DIABETES SCAN":
