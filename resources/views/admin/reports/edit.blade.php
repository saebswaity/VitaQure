...
<div class="card card-primary card-outline">
  ...
  <div class="card-body">
              ...
              <table class="table table-hover table-bordered">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <!--  <input type="text" name="result[{{$result['id']}}][result]" class="form-control" value="{{$result['result']}}">-->
                    <!--  @else-->
    ...
    @else
        <select name="result[{{$result['id']}}][result]" id="" class="form-control select_result">
            {{-- <option value="Nill" @if(empty($result['result'])) selected @endif>Nill</option> --}}
            @foreach($result['component']['options'] as $option)
                <option value="{{$option['name']}}" @if($option['name'] == $result['result']) selected @endif>{{$option['name']}}</option>
            @endforeach
             Deleted option
            @if(!$result['component']['options']->contains('name', $result['result']) && !empty($result['result']))
                <option value="{{$result['result']}}" selected>{{$result['result']}}</option>
            @endif
             \Deleted option
        </select>
    @endif
                    ...
                    <td style="width:10px" class="text-center">
                      ...
                      <select name="result[{{$result['id']}}][status]" class="form-control select_result">
                        ...
                        <option value="Sensitive" @if($result['status']=='High' ) selected @endif>Sensitive </option>
                        <option value="Intermediate" @if($result['status']=='Normal' ) selected @endif>Intermediate </option>
                        <option value="Resistant" @if($result['status']=='Low' ) selected @endif>Resistant </option>
                        <!-- New status -->
                        @if(!empty($result['status'])&&!in_array($result['status'],['Sensitive','Intermediate','Resistant']))
              ...
              <div class="row">
                ...
                <div class="col-lg-4">
                  <div class="form-group">
                    ...
                    <select class="form-control select2" name="culture_options[{{$culture_option['id']}}]" id="culture_option_{{$culture_option['id']}}" onchange="toggleCustomInput({{$culture_option['id']}})">
                    ...
                    <input value="{{$option['value']}}" type="text" class="form-control mt-2" name="culture_options_custom[{{$culture_option['id']}}]" id="custom_input_{{$culture_option['id']}}" placeholder="Enter your custom value" style="display: none;">
                  </div>
                </div>
                @endif
                @endforeach
                  ...
                  <table class="table table-hover table-bordered">
                    <thead>
                      <tr>
                        ...
                        <th width="20px">
                          <button type="button" class="btn btn-primary btn-sm" onclick="add_antibiotic('{{$select_antibiotics}}',this)">
                      ...
                      <tr>
                        <td>
                          <select class="form-control antibiotic" name="antibiotic[{{$antibiotic_count}}][antibiotic]" >
                            ...
                            <option value="{{$select_antibiotic['id']}}" @if($select_antibiotic['id']==$antibiotic['antibiotic_id']) selected @endif>
                        ...
                        <td>
                          <select class="form-control" name="antibiotic[{{$antibiotic_count}}][sensitivity]" required>
                            ...
                            <option value="Sensitive" @if($antibiotic['sensitivity']=='Sensitive' ) selected @endif>Sensitive</option>
                            <option value="Intermediate" @if($antibiotic['sensitivity']=='Intermediate' ) selected @endif>Intermediate</option>
                            <option value="Resistant" @if($antibiotic['sensitivity']=='Resistant' ) selected @endif>Resistant</option>
                          </select>
...
<script>
  function toggleCustomInput(id) {
    var selectElement = document.getElementById('culture_option_' + id);
    var customInput = document.getElementById('custom_input_' + id);
