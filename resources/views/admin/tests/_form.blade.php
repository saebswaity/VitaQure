<div class="row d-flex justify-content-around">
    ...
    <div class="col-lg-2">
        <div class="form-group">
            <label for="sample_type">Catogery</label>
            <div class="input-group mb-3">
                <select class="form-control" id="catogery" name="category_id" required >
                    <option value="" disabled selected>Choose...</option>
                    @foreach($catogeryTests as $catogeryTest)

                    <option value="{{$catogeryTest->id}}" @if(isset($test) && $catogeryTest->id == $categoryId) selected @endif>{{$catogeryTest->catogery}}</option>

                    @endforeach


                </select>
                <input type="hidden" name="catogery_id" value="{{ old('catogery_id') }}">
            </div>
